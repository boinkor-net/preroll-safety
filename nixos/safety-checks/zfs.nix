{
  ...
  options = {
    preroll-safety.stockChecks.zfs.package = lib.mkOption {
      type = lib.types.package;
      default = pkgs.zfs;
      description = "Package of ZFS tools to use for the `zfs` stock check";
    };
  };
  config = {
    preroll-safety.checks.zfs = {
      ...
      check = {
        items = lib.mapAttrsToList (_n: fs: fs.device) (
          lib.filterAttrs (_n: fs: fs.fsType == "zfs") config.fileSystems
        );

        program = lib.getExe (
          pkgs.writeShellApplication {
            name = "check-zfs-filesystem-entry";
            runtimeInputs = [config.preroll-safety.stockChecks.zfs.package];
            text = ''
